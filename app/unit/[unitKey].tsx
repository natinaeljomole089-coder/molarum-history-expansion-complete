...
export default function UnitDetailScreen() {
  ...
  return <FlatList
    ...
    ListHeaderComponent={<View style={styles.headerStack}>
      ...
      <View style={styles.filters}>{FILTERS.map((filter) => <Pressable key={filter.value} onPress={() => setDifficulty(filter.value)} accessibilityRole="button" accessibilityState={{ selected: difficulty === filter.value }} style={({ pressed }) => [styles.filter, { backgroundColor: difficulty === filter.value ? "#3B267B" : colors.surface, borderColor: difficulty === filter.value ? "#9F7AFF" : colors.border }, pressed && styles.pressed]}><Text style={[styles.filterText, { color: difficulty === filter.value ? "#E6D9FF" : colors.muted }]}>{filter.label}</Text></Pressable>)}</View>
      <View style={[styles.timerCard, { backgroundColor: colors.surface, borderColor: colors.border }]}><View style={styles.timerIcon}><MaterialIcons name="timer" size={21} color="#FFAE51" /></View><View style={styles.timerCopy}><Text style={[styles.timerTitle, { color: colors.foreground }]}>Timed practice</Text><Text style={[styles.timerDetail, { color: colors.muted }]}>Track how long your session takes.</Text></View><Switch value={timed} onValueChange={setTimed} trackColor={{ false: "#29314D", true: "#6C42DE" }} thumbColor={timed ? "#FFFFFF" : "#B4BDD0"} accessibilityLabel="Toggle timed practice" /></View>
