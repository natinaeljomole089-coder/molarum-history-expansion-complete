...
export default function QuestionBankScreen() {
  ...
  const reset = () => {
    ...
    Alert.alert("Restore packaged validated bank?", `Current: ${currentBank}. Replacement: packaged validated bank (${bundledQuestionCount} questions). This removes the imported override when present and ${reviewCount} local question-visibility setting${reviewCount === 1 ? "" : "s"}. Local revision records are kept; export them from Records first if needed.`, [{ text: "Cancel", style: "cancel" }, { text: "Restore packaged bank", style: "destructive", onPress: () => { void resetToBundledContent().then(() => setPendingImport(null)).catch(() => Alert.alert("Restore unavailable", "The current local bank was kept because the packaged bank could not be prepared.")); } }], { cancelable: false });
  };
