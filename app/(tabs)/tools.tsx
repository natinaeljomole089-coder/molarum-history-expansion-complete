...
const TOOL_LINKS = [
  ...
  { title: "Teacher review", detail: "Set Draft, Approved, or Hidden states on this device.", route: "/teacher-review" },
...
export default function ToolsScreen() {
  ...
  return <FlatList style={{ backgroundColor: colors.background }} contentContainerStyle={styles.content} data={TOOL_LINKS} keyExtractor={(item) => item.title} ListHeaderComponent={<View style={{ gap: 11 }}><NotebookHeader eyebrow="Content desk" title="Tools" subtitle="Local controls for content governance, review, and documentation." /><Notice>Invalid question-bank files are rejected before any active local content changes.</Notice><SectionLabel>Content management</SectionLabel></View>} renderItem={({ item }) => <LinkRow title={item.title} detail={item.detail} onPress={() => router.push(item.route as never)} />} />;
}
