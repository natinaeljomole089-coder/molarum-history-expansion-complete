...
export default function RecordsScreen() {
  ...
  const exportPdf = async () => { try { await exportScoreHistoryPdf(profile, attempts); } catch { Alert.alert("Export unavailable", "The PDF could not be prepared on this device."); } };
  return <FlatList style={{ backgroundColor: colors.background }} contentContainerStyle={styles.content} data={attempts} keyExtractor={(item) => item.id} ListHeaderComponent={<View style={{ gap: 12 }}><NotebookHeader eyebrow="Device-only records" title="Learner records" subtitle="Optional learner details and revision scores never leave this device automatically." /><Notice>Keep this information only if it is appropriate for the learner’s device. Molarum has no account or cloud sync.</Notice><SectionLabel>Optional learner profile</SectionLabel><View style={styles.form}>{([ ["name", "Learner name"], ["className", "Class"], ["school", "School"] ] as const).map(([key, label]) => <TextInput key={key} value={profile[key]} onChangeText={(value) => setProfile((current) => ({ ...current, [key]: value }))} placeholder={label} placeholderTextColor={colors.muted} style={[styles.input, { borderColor: colors.border, backgroundColor: colors.surface, color: colors.foreground }]} returnKeyType="done" accessibilityLabel={label} />)}<ActionButton label="Save local profile" secondary icon="save" onPress={save} /></View><View style={styles.recordHeading}><SectionLabel>Score history</SectionLabel><StatusPill label={`${attempts.length} attempt${attempts.length === 1 ? "" : "s"}`} /></View><ActionButton label="Export score history to PDF" icon="picture-as-pdf" onPress={exportPdf} /></View>} renderItem={({ item }) => <View style={[styles.attempt, { borderBottomColor: colors.border }]}><View style={{ flex: 1, gap: 3 }}><Text style={[styles.attemptTitle, { color: colors.foreground }]}>{item.unitTitle}</Text><Text style={[styles.attemptDetail, { color: colors.muted }]}>{new Date(item.completedAt).toLocaleDateString()} · {item.timed ? "Timed" : "Untimed"}</Text></View><Text style={[styles.attemptScore, { color: colors.primary }]}>{item.correct}/{item.total}</Text></View>} ListEmptyComponent={<Text style={[styles.empty, { color: colors.muted }]}>No completed revision quizzes are stored yet.</Text>} />;
}
