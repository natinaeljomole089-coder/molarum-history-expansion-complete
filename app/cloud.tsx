...
export default function CloudScreen() {
  ...
  return <FlatList style={{ backgroundColor: colors.background }} contentContainerStyle={styles.content} data={[]} renderItem={() => null} ListHeaderComponent={<View style={{ gap: 12 }}><NotebookHeader onBack={() => router.back()} eyebrow="Optional cloud layer" title="Account & sync" subtitle="Sign in only when you want private cloud sync or validated shared content." /><Notice>Your quizzes work without an account. Molarum never uploads attempts, profile details, reports, or source files automatically.</Notice><SectionLabel>Account</SectionLabel>{accountSection}{cloud.session ? <><SectionLabel>My device records</SectionLabel><ActionButton label="Sync my local records" icon="sync" disabled={busy} onPress={() => run(cloud.syncMyRecords)} /><ActionButton label="Download active shared content" secondary icon="cloud-download" disabled={busy} onPress={() => run(cloud.pullActiveTeacherContent)} /></> : null}{cloud.isTeacher ? <><SectionLabel>Validated content publishing</SectionLabel><Notice>Publishing keeps the current local validated bank unchanged on this device. Learners can only download the active published version.</Notice><ActionButton label="Publish active question bank" icon="publish" disabled={busy} onPress={() => run(cloud.publishActiveQuestionBank)} /><ActionButton label="Upload a private source PDF" secondary icon="upload-file" disabled={busy} onPress={() => run(cloud.pickAndUploadSourceMaterial)} /></> : null}</View>} />;
}
